    ...
    if request.method == "POST":
        ...
        if len(nombre_cargo) < 25:   
            if nombre_cargo not in nombre_existen:
                cargo_uso = request.session.get("cargo_perfil")     
                cargo_edit.nombre_cargo = nombre_cargo
                cargo_edit.permiso = permiso_cargo
                cargo_edit.save()
                if nombre_cargo == cargo_uso:
                    request.session["cargo_perfil"] = nombre_cargo
            ...
            if len(nombre) < 20:
                if cargo_select != "":
                    if nombre_perfil not in nombre_existen:
                        ...
                        perfil_edit.save()
                        usuario_uso = request.session.get("name_perfil")
                        if nombre_perfil == usuario_uso:
